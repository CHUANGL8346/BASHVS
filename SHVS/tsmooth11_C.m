...
%    Iin = log(Iin+eps); 
   ...
   fx = diff(fin,1,2); % 计算平滑后影像的差分影像
   fx = padarray(fx, [0 1 0], 'post'); % padding
   fy = diff(fin,1,1); % 计算平滑后影像的差分影像，为什么要分x，y去padding？
   fy = padarray(fy, [1 0 0], 'post');% padding
...
%    bwx = bwxt; bwy = bwyt;
   ...
   mgi = sqrt(fxi.^2+fyi.^2); mg = sqrt(fx.^2+fy.^2);% 表示对数亮度中的强度梯度
   bfxy = mgi < mg; %0.0005 % 公式6中的判断条件；mg:Gp(t-1),mgi:Gp；包含了公式5
   fx(bfxy) = fxi(bfxy); fy(bfxy) = fyi(bfxy);% mg:Gp(t-1),mgi:Gp
   
   bwxt = ones(size(fx)); bwyt = ones(size(fy));
   bwxt(bfxy) = (mg(bfxy)-mgi(bfxy))./mgi(bfxy); % mg:Gp(t-1),mgi:Gp% 公式6中的去了mu的部分
%    if iter>4
%        mu = 1000;
%    else
       mu = 0.1*(2^(iter-1)); %0.1*(2^(iter-1)) 
%    end
   bwxt(bfxy) = exp(-bwxt(bfxy)/mu); bwyt = bwxt; % 公式6，计算惩罚引导梯度的权重
%    bwxt(bfxy) = 1;  bwyt(bfxy) = 1;
   bwx = bwxt; bwy = bwyt; % 公式6，计算惩罚引导梯度的权重
%    figure, imshow(bwx)
